...
class Commission(Base):
    ...
    commission_rate: Mapped[float] = mapped_column(Float, nullable=False)  # As decimal (0.08 = 8%)
    ...
    click_timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
    conversion_timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)

    # Status
    is_pending: Mapped[bool] = mapped_column(Boolean, default=True, index=True)  # Awaiting conversion confirmation
