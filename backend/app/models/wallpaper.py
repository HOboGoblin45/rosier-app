...
class WallpaperHouse(Base):
    ...
    contract_end: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    # Relationships
    patterns: Mapped[list["WallpaperPattern"]] = relationship(
        "WallpaperPattern", back_populates="house"
    )
    impressions: Mapped[list["WallpaperImpression"]] = relationship(
        "WallpaperImpression", back_populates="house"
    )
...
class WallpaperPattern(Base):
    ...
    house_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("wallpaper_houses.id"), nullable=False
    )
    ...
    pattern_type: Mapped[PatternType] = mapped_column(
        SQLEnum(PatternType), nullable=False
    )

    # Color palette (light theme)
    primary_color_light: Mapped[str] = mapped_column(
        String(7), default="#FFFFFF", nullable=False
    )  # Hex color
    secondary_color_light: Mapped[Optional[str]] = mapped_column(
        String(7), nullable=True
    )

    # Color palette (dark theme)
    primary_color_dark: Mapped[str] = mapped_column(
        String(7), default="#000000", nullable=False
    )
    secondary_color_dark: Mapped[Optional[str]] = mapped_column(
        String(7), nullable=True
    )
    ...
    style_archetypes: Mapped[list[str]] = mapped_column(
    ...
    # Relationships
    house: Mapped["WallpaperHouse"] = relationship(
        "WallpaperHouse", back_populates="patterns"
    )
    impressions: Mapped[list["WallpaperImpression"]] = relationship(
        "WallpaperImpression", back_populates="pattern"
    )
...
class WallpaperImpression(Base):
    ...
    pattern_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("wallpaper_patterns.id"), nullable=False
    )
    house_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("wallpaper_houses.id"), nullable=False
    )
    ...
    )  # Time viewed in milliseconds

    # Relationships
    pattern: Mapped["WallpaperPattern"] = relationship(
        "WallpaperPattern", back_populates="impressions"
    )
    house: Mapped["WallpaperHouse"] = relationship(
        "WallpaperHouse", back_populates="impressions"
    )
